...
from typing import Dict, List, Type, Union, Optional, MutableMapping
...
class Gestalt:
    ...
    def __flatten(self, d: MutableMapping, parent_key: str = '', sep: str = '.'):
        items: list = []
        for k, v in d.items():
    ...
    def __get(self, key: str,
        ...
        if key in self.__conf_sets:
            ...
            if not isinstance(val, t):
                raise TypeError(f'Given set key is not of type {t}, but of type {type(val)}')
            return val
        if self.__use_env:
        ...
        if key in self.__conf_defaults:
            ...
            if not isinstance(val, t):
                raise TypeError(f'Given default set key is not of type {t}, but of type {type(val)}')
            return val
    ...
    def get_string(self, key: str, default: str = None) -> str:
        ...
        if not isinstance(val, str):
            raise RuntimeError(f'Gestalt error: expected to return string, but got {type(val)}')
        return val
    ...
    def get_float(self, key: str, default: float = None) -> float:
        val: Union[str, int, float, bool, list] = self.__get(key, default, float)
        if not isinstance(val, float):
            raise RuntimeError(f'Gestalt error: expected to return float, but got {type(val)}')
        return val

    def get_bool(self, key: str, default: bool = None) -> bool:
        val: Union[str, int, float, bool, list] = self.__get(key, default, bool)
        if not isinstance(val, bool):
            raise RuntimeError(f'Gestalt error: expected to return bool, but got {type(val)}')
        return val

    def get_list(self, key: str, default: list = None) -> List:
        val: Union[str, int, float, bool, list] = self.__get(key, default, list)
        if not isinstance(val, list):
            raise RuntimeError(f'Gestalt error: expected to return list, but got {type(val)}')
        return val
